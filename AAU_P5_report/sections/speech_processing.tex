...
For a better understanding of the process, a visual representation of the word "Hello" as it is formed for the input of the neural network is shown in the upper part of Figure\ref{fig:cyka} where it is sampled at 8000 Hz.
...
\end{lstlisting}
\todo{Explain how the windows work and the non-isolationist behaviour of human speech.}

Either 13 or 26 different cepstral features, as input for the model. After this transformation the data is stored as a matrix of frequency coefficients (rows) over time (columns).
