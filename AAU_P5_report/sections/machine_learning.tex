...
In Figure \ref{fig:AN} it can be seen that the neuron computes a weighted sum of all its weights and biases, which they are passed onto the activation function to be mapped, analogous to the biological function of the Soma and Axon respectively.
...
\begin{equation}
    C_t= f * C_{t-1} + u*X` 
\end{equation}
Where the current state ($C_t$) is determined by what was forgotten ($f$), multiplied by the previous state ($C_t-1$) and added together with the update gate ($r$) times the input layer ($X`$).
\begin{figure}[H]
    \centering
        \includegraphics[width=0.5\textwidth]        
...
\end{equation}
%---------------------------------------------------
\subsection{Bi-directional Neural Networks}\label{sub:BiRNN}
...
These equations expand on Section \ref{sec:ANN} and show how the neurons output a weighted sum of all the inputs and biases.
Because of the promising results given by simple LSTM networks, combining them with the benefits given by BiRNN, the gates showcased in 
...
\end{figure}
%---------------------------------------------------
\section{Types of Regulation Techniques}
\subsection{Dropout}
...
\end{table}    
    %-------------------------------------------------    
The final results show that Test1 with the smaller batch size took significantly longer to train and had the worst accuracy. However, Test3 with the larger batch size took the least amount of time and had the same accuracy as Test2.
%-----------------------------------------------------
...
\end{table}        
    %-------------------------------------------------
The following graph (Figure \ref{fig:learning_train_error_fig}) and
...
$0.1\%$ outperformed Test6 with a learning rate of $1\%$ by a long shot. The graph clearly shows the model didn't converge to zero at all, unlike Test5, meaning it didn't learn anything during training.
