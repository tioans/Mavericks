...
In Figure \ref{fig:AN} it can be seen that the neuron computes a weighted sum of all its weights and biases, which they are passed onto the activation function to be mapped, analogous to the biological
...
BiRNNs do not require their input data to be fixed. Furthermore, their 
...
\begin{lstlisting}[language=Python, label={lst:output}, caption=Printing output without sess.run().]
...
will be used for the batch size test has shown in Table \ref{tab:batch_tests_tab}. For each trial test only the batch size parameter as been changed.
