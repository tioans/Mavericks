...
\todo{maybe get color version of pictures}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth]	
...
\todo{Recreate the gates as mathematical functions and explain them}

A side by side description of the each layer of a LSTM and it`s mathematical formulation can be seen below.
...
substracting the average and dividing by the stdev to rescale and recentre the values epsilon for numerical stability, avoid division by 0
add two degrees of freedom per neuron
Statistics can be computed for each batch that is fed to the neural network so the average and the standard deviation can be computed. After computing them, substracting the average and dividing by the standard deviation rescales and recentre the output. 
...
To solve those cases, two degrees of freedom are added for each neuron $\alpha$ and $\beta$.
\begin{equation}
BN(x) = \alpha * X + \beta
\end{equation}
With the two constant
