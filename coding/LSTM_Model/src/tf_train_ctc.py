...
from models.RNN.rnn import LSTM as LSTM_model
...
class Tf_train_ctc(object):
    ...
    def load_configs(self):
        ...
        # determine if the data input order should be shuffled after every epoc
    ...
    def run_model(self):
        ...
        with self.graph.as_default(), tf.device('/cpu:0'):
            ...
            #uc = tf_config.gpu_options.per_process_gpu_memory_fraction = \
    ...
    def load_placeholder_into_network(self):
        ...
        if self.network_type == 'SimpleLSTM':
            ...
            self.logits, summary_op = BiRNN_model(
             ...
             self.logits, summary_op = LSTM_model(
                self.conf_path,
                self.input_tensor,
                tf.to_int64(self.seq_length),
                self.n_input,
        ...
        elif self.network_type == 'BiRNN_V2':
             self.logits, summary_op = BiRNN_V2_model(
                self.conf_path,
                self.input_tensor,
                tf.to_int64(self.seq_length),
                self.n_input,
        ...
        elif self.network_type == 'BiRNN_V3':
             self.logits, summary_op = BiRNN_V3_model(
                self.conf_path,
                self.input_tensor,
                tf.to_int64(self.seq_length),
                self.n_input,
