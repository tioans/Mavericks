...
def BiRNN_V2(conf_path, batch_x, seq_length, n_input, n_context):
    ...
    # The next two blocks will pass `batch_x` through two hidden layers with
    # clipped RELU activation and dropout.
    ...
    with tf.name_scope('lstm'):
        ...
        # `layer_2` is now reshaped into `[n_steps, batch_size, 2*n_cell_dim]`,
        # as the LSTM BRNN expects its input to be of shape `[max_time, batch_size, input_size]`.
        layer_2 = tf.reshape(layer_2, [-1, batch_x_shape[0], n_hidden_3])

        # Now we feed `layer_2` into the LSTM BRNN cell and obtain the LSTM BRNN output.
